...
`;function d(){document.getElementById("app").innerHTML=c,x=document.getElementById("app");var e=document.createElement("div");e.setAttribute("class","nav-button"),e.textContent="Main Content Here",x.appendChild(e)}function $(){try{f().then(e=>{e=="Already Logged In"&&d(),s.innerText=e}).catch(e=>{console.error(e)})}catch(e){console.error(e)}}document.querySelector("#app").innerHTML=`
...
`;n.forEach(l=>{o+=`<button style="font-size:20px" onclick="openfile('')"><i class="fa fa-file" style="font-size:20px">  ${l[0]}</button>`}),o+="</div>";let i=n[0],r=`
<div>
...
`+t}).catch(e=>{console.error(e)})}catch(e){console.error(e)}};window.openPrompt=function(){let e=prompt("Enter Workspace Name");m(e).then(t=>{addNewContent()}).catch(t=>{console.error(t)})};window.dispFiles=function(e,t){console.log(e,t);try{h(e,t).then(n=>{if(n[0][0]=="0")document.querySelector("#app").innerHTML=c;else{let o=`<div style="display: table-cell" class="left-corner">
`;n[0].length!=0&&n.forEach(l=>{o+=`<button style="font-size:20px" onclick="openfile('${l[1]}')"><i class="fa fa-file" style="font-size:20px">  ${l[0]}</button>`}),o+="</div>";let i=`
<div>
                        <button id='${n[0][2]}' onclick="openUserAddPrompt(this.id)">Add User</button
                    </div>`;o+=i,o+=`
<div>
